    ...
    public bool LoadSongs(IProgress<Tuple<int, int>> progress)
    {
        ...
        foreach (var entry in allEntries)
        {
            foreach (var song in entry.Value)
            {
                ...
                Song outputSong = new Song(song.Artist, song.Name, song.Source.Original, $"{song.Hash}");

                if (chart.GetCodaEvent() != null)
                {
                    outputSong.HasBRE = true;
                }

                foreach (int diff in Enum.GetValues<Song.Difficulties>())
                {
                    if (diff == 0)
                    {
                        continue;
                    ...
                    var yargDiff = (Difficulty) diff;
                    if (chart.FiveFretGuitar.TryGetDifficulty(yargDiff, out var guitarDiff))
                    {
                        // outputSong.HasBRE = diff.Phrases.Any(x => x.Type == PhraseType.BigRockEnding),
                        outputSong.GuitarLanes[diff] = guitarDiff.Phrases.Any(x =>
                            x.Type == PhraseType.TremoloLane || x.Type == PhraseType.TrillLane);
                    }

                    if (chart.FourLaneDrums.TryGetDifficulty(yargDiff, out var drumsDiff))
                    {
                        outputSong.DrumsLanes[diff] = drumsDiff.Phrases.Any(x =>
                            x.Type == PhraseType.TremoloLane || x.Type == PhraseType.TrillLane);
                    }

                    if (chart.FiveFretBass.TryGetDifficulty(yargDiff, out var bassDiff))
                    {
                        outputSong.BassLanes[diff] = bassDiff.Phrases.Any(x =>
                            x.Type == PhraseType.TremoloLane || x.Type == PhraseType.TrillLane);
                    }

                    if (chart.Keys.TryGetDifficulty(yargDiff, out var keysDiff))
                    {
                        outputSong.KeysLanes[diff] = keysDiff.Phrases.Any(x =>
                            x.Type == PhraseType.TremoloLane || x.Type == PhraseType.TrillLane);
                    }

                    if (chart.ProKeys.TryGetDifficulty(yargDiff, out var proKeysDiff))
                    {
                        outputSong.ProKeysLanes[diff] = proKeysDiff.Phrases.Any(x =>
                            x.Type == PhraseType.TremoloLane || x.Type == PhraseType.TrillLane);
                    }
                }

                if (outputSong.HasBRE || outputSong.HasLanes)
                {
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        Songs.Add(outputSong);
                    });
                }
